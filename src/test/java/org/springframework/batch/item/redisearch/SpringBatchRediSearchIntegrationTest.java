...
public class SpringBatchRediSearchIntegrationTest {
	...
	private IndexCreateStep<String, String> indexCreateStep() {
		IndexCreateStep<String, String> step = IndexCreateStep.builder().redisURI(redisURI).index(Utils.INDEX)
				.schema(Utils.SCHEMA).build();
		step.setJobRepository(jobRepository);
		return step;
	}

	private <I, O> void run(String name, ItemReader<I> reader, ItemProcessor<I, O> processor, ItemWriter<O> writer,
			Step... steps) throws Exception {
		TaskletStep taskletStep = stepBuilderFactory.get(name + "-step").<I, O>chunk(10).reader(reader)
	...
	private void writeDocuments(String name) throws Exception {
		RediSearchItemWriter<String, String> writer = RediSearchItemWriter.builder().redisURI(redisURI)
				.index(Utils.INDEX).build();
	...
	public void testDocumentReader() throws Exception {
		...
		RediSearchItemReader<String, String> reader = RediSearchItemReader.builder().redisURI(redisURI)
				.index(Utils.INDEX).query("*")
				.searchOptions(SearchOptions.<String>builder().limit(Limit.builder().num(3000).build()).build()).build();
	...
	private void writeSuggestions(String name) throws Exception {
		RediSearchSuggestItemWriter<String, String> writer = RediSearchSuggestItemWriter.builder().redisURI(redisURI)
				.key(Utils.SUGGEST_KEY).build();
	...
	public void testSuggestionReader() throws Exception {
		...
		RediSearchSuggestItemReader<String, String> reader = RediSearchSuggestItemReader.builder().redisURI(redisURI)
				.key(Utils.SUGGEST_KEY).prefix("fren").suggetOptions(SuggetOptions.builder().fuzzy(true).build())
				.build();
